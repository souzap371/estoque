...
public class DashboardController {
    ...
    public String dashboard(Model model) {
        ...
        // =================================================
        // 5. BONIFICAÇÕES (Long) — NOVO
        // =================================================
        Long bonifAtual = vendaService.getBonificacaoMesAtual();
        Long bonifAnt = vendaService.getBonificacaoMesAnterior();
        BigDecimal varBonif = vendaService.calcularVariacaoPercentual(bonifAtual, bonifAnt);
        model.addAttribute("kpiBonificacaoValor", bonifAtual);
        model.addAttribute("kpiBonificacaoVariacao", varBonif);
        model.addAttribute("kpiBonificacaoPositiva", varBonif.compareTo(BigDecimal.ZERO) >= 0);
        model.addAttribute("sparklineBonificacao", calcularAlturasLong(vendaService.getSparklineBonificacao()));

        // 6. Resumo Financeiro (mês atual por padrão)
        model.addAttribute("resumo", vendaService.getResumoFinanceiro("mes"));

        return "Dashboard";
    }

    @GetMapping(value = "/api/resumo-financeiro", produces = "application/json")
    @ResponseBody
    public ResumoFinanceiroDTO resumoFinanceiroApi(@RequestParam(defaultValue = "mes") String periodo) {
        return vendaService.getResumoFinanceiro(periodo);
    }
    ...
    // =================================================
    // Helper para Long (Pedidos, Clientes, Produtos, Bonificações)
    // =================================================
