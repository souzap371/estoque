...
public interface VendaRepository extends JpaRepository<Venda, Long> {
        ...
        // =================================================
        // 📊 KPIs - total bonificação (mês atual)
        // =================================================
        @Query("""
                            SELECT COALESCE(SUM(vi.quantidade),0)
                            FROM VendaItem vi
                            JOIN vi.venda v
                            WHERE vi.bonificacao = true
                              AND MONTH(v.dataSaida)=MONTH(CURRENT_DATE)
                              AND YEAR(v.dataSaida)=YEAR(CURRENT_DATE)
                        """)
        Long totalBonificacaoMesAtual();

        // =================================================
        // 📊 KPIs - total bonificação (mês anterior)
        // =================================================
        @Query("""
                            SELECT COALESCE(SUM(vi.quantidade),0)
                            FROM VendaItem vi
                            JOIN vi.venda v
                            WHERE vi.bonificacao = true
                              AND MONTH(v.dataSaida)=MONTH(CURRENT_DATE)-1
                              AND YEAR(v.dataSaida)=YEAR(CURRENT_DATE)
                        """)
        Long totalBonificacaoMesAnterior();

        // =================================================
        // 📊 KPIs - SPARKLINE Bonificação por dia
        // ================================================
        @Query(value = """
                        SELECT COALESCE(SUM(vi.quantidade),0)
                        FROM venda_item vi
                        ...
                        ON vi.venda_id = v.id
                        WHERE vi.bonificacao = true
                        AND v.data_saida = CURRENT_DATE - CAST(:dias AS INTEGER)
                        GROUP BY v.data_saida
                        """, nativeQuery = true)
        Long totalBonificacaoPorDia(int dias);
